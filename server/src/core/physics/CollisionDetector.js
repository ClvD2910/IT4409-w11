...
export class CollisionDetector {
    ...
    checkPlayers(qt, foodQt) {
        ...
        this.game.players.forEach(player => {
            ...
            this.checkPlayerVsFoods(player, foods, foodQt);
    ...
    checkPlayerVsFoods(player, foods, foodQt) {
        const playerRadius = player.radius || SHIP_RADIUS;
        const checkR = playerRadius + FOOD_RADIUS;

        // Use quadtree if available for O(log n) lookup
        if (foodQt) {
            const range = {
                x: player.x - checkR,
                y: player.y - checkR,
            ...
            const candidates = foodQt.query(range);
            for (const point of candidates) {
                const food = point.userData;
                if (distance(player.x, player.y, food.x, food.y) < checkR) {
