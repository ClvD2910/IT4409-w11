...
export class EntityManager {
    constructor(scene) {
        ...
        // Projectile pool: id -> sprite/container
    ...
    updateProjectiles(projectilesData) {
        ...
        projectilesData.forEach(p => {
            ...
            if (p.weaponType === 'BOMB') {
                // Bombs are containers with animated state – always recreate when state changes
                const existing = this.projectileSprites[p.id];
                const triggered = p.isTriggered ? 1 : 0;
                if (existing && existing._triggered === triggered) {
                    // Snap to authoritative server position
                    existing.x = p.x;
                    existing.y = p.y;
                    // Update slow spin via direct rotation (no tween needed)
                    const bombSprite = existing.list && existing.list[0];
                    if (bombSprite) {
                        const spinSpeed = p.isTriggered ? 20 + (p.triggerProgress || 0) * 40 : 1;
                        bombSprite.setRotation(Date.now() * spinSpeed * 0.001);
                    }
                } else {
                    if (existing) { existing.destroy(); }
                    const container = this.renderBomb(p);
                    if (container) {
                        container._triggered = triggered;
                        // Bombs don't move, no velocity needed
                        container._vx = 0;
                        container._vy = 0;
                        this.projectileSprites[p.id] = container;
                    }
                }
            } else {
                ...
                if (this.projectileSprites[p.id]) {
                    // Snap to authoritative server position, keep velocity for extrapolation
                    const laser = this.projectileSprites[p.id];
                    laser.x = p.x;
                    laser.y = p.y;
                    ...
                    this.projectileSprites[p.id] = laser;
                }
            }
        });

        // Destroy sprites for projectiles no longer on server
    ...
    /**
     * Called every frame to extrapolate projectile positions between server updates.
     * Moves each projectile forward at its stored velocity so motion looks smooth at 60fps.
     */
    tickProjectiles(dt) {
        for (const sprite of Object.values(this.projectileSprites)) {
            if (sprite._vx !== 0 || sprite._vy !== 0) {
                sprite.x += sprite._vx * dt;
                sprite.y += sprite._vy * dt;
            }
        }
    }
